...
class DarkAdmin {
	...
	/**
	 * Helper function to replace enqueued stylesheets with a version in the plugin.
	 *
	 * Replaces or overrides the enqueued stylesheet in the $wp_styles global.
	 *
	 * @since x.x.x
	 * @access (private)
	 *
	 * @global type $wp_styles Enqueued stylesheets.
	 *
	 * @param type $handle. Name of the stylesheet to override.
	 * @param type $css_file Filename of the CSS used to replace (e.g. file.css).
	 * @param type $suffix Optional. Set a suffix for the stylesheet.
	 */
	private function replace_css( $handle, $css_file, $suffix = null ) {
		global $wp_styles;
		$wp_styles = wp_styles();
		$wp_styles->registered[ $handle ]->src = plugins_url( 'css/' . $css_file, __FILE__ );
		$wp_styles->registered[ $handle ]->ver = filemtime( plugin_dir_path( __FILE__ ) . 'css/' . $css_file );
		if ( ! is_null( $suffix ) ) {
